...
lines = open('bps_cards.tsv','r', encoding='utf-8').readlines()
...
models = [
    ...
    {"predicate": {"custom_model_data": 4258996},"model": "bps_tcg:cards/quit"},
    {"predicate": {"custom_model_data": 4258996},"model": "bps_tcg:cards/cancel"},
    {"predicate": {"custom_model_data": 4258997},"model": "bps_tcg:cards/done"},
    {"predicate": {"custom_model_data": 4258998},"model": "bps_tcg:block/table"},
    {"predicate": {"custom_model_data": 4258999},"model": "bps_tcg:cards/slot"}
]

for i in range(1, len(lines)):
    cardType : str = lines[i].split('\t')[2]
    url : str = lines[i].split('\t')[9]
    print(url)
    ip = f'../resources/assets/bps_tcg/textures/cards/generated/{i}.png'
    os.system(f'curl -o "{ip}" {url}')

    im : Image = Image.open(ip, 'r')

    if im.width != im.height:
        im = im.resize((im.width, im.width), Image.NEAREST)

    if len(im.getpixel((0,0))) == 4:
        for y in range(im.width):
            for x in range(im.width):
                pC = im.getpixel((x,y))
                pC = (pC[0], pC[1], pC[2], 255 if pC[3] > 127 else 0)
                im.putpixel((x,y), pC)

    im.save(ip)

    open(f'../resources/assets/bps_tcg/models/cards/generated/{i}.json', 'w+').write(
        json.dumps(
            {
                "parent": f"bps_tcg:cards/blank_{cardType.lower()}",
                "textures":{
                    "image": f'bps_tcg:cards/generated/{i}'
                }
            }, 
            indent= 4
        )
    ...
    models.append({"predicate": {"custom_model_data": 4259000 + i},"model": f"bps_tcg:cards/generated/{i}"},)

open(f'../resources/assets/minecraft/models/item/clock.json', 'w+').write(
    json.dumps(
        {
